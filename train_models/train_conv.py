...
def run_training():
  ...
  batch = train_data.next_batch(30)

  test_data = InputData.InputData('Bedroom_01_2');
  ...
  with tf.Graph().as_default():
    ...
    logits = conv2_fc1.inference(InputData.InputData.num_cats, images_placeholder,
                             FLAGS.conv1,
                             FLAGS.conv2,
                             FLAGS.fc1)

    # Add to the Graph the Ops for loss calculation.
    loss = conv2_fc1.loss(logits, labels_placeholder)

    # Add to the Graph the Ops that calculate and apply gradients.
    train_op = conv2_fc1.training(loss, FLAGS.learning_rate)

    # Add the Op to compare the logits to the labels during evaluation.
    eval_correct = conv2_fc1.evaluation(logits, labels_placeholder)
    
    tf.scalar_summary('loss', loss)

    tf.image_summary('input_image',images_placeholder,max_images=3)

    ass = tf.get_default_graph().get_tensor_by_name('conv1_w:0')
    butt = tf.transpose(ass, [3,0,1,2])


    tf.image_summary('conv1',butt,max_images=3)
